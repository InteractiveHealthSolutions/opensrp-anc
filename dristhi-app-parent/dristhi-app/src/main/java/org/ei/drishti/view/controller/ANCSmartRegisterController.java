    ...
    private void sortByName(List<ANCClient> ancClients) {
        sort(ancClients, new Comparator<ANCClient>() {
            ...
            public int compare(ANCClient oneANCClient, ANCClient anotherANCClient) {
            	int result=0;
            	if(oneANCClient.wifeName()==null)
            	{
            	
            		if(anotherANCClient.wifeName()==null)
            		{
            			
            		return 	"".compareToIgnoreCase("");
            		}else
            		{
            		return "".compareToIgnoreCase(anotherANCClient.wifeName());
            		}
            	}
            	else if(anotherANCClient.wifeName()==null)
            	{
            		return oneANCClient.wifeName().compareToIgnoreCase("");
            	}else{
